...
/// <summary>Pushes the pending-approvals widget's live count to whoever a new approval step just
/// landed on — deliberately pushes the *nominal* approver here (unlike the notification-toast
/// handler, which resolves delegation), since <see cref="RealtimeApprovalsCounter"/> resolves
/// delegation itself when computing the count for whichever employee actually receives it.</summary>
public sealed class ApprovalStepAssignedDashboardHandler : INotificationHandler<DomainEventNotification<ApprovalStepAssigned>>
{
    ...
    public ApprovalStepAssignedDashboardHandler(
    ...
    public async Task Handle(DomainEventNotification<ApprovalStepAssigned> notification, CancellationToken cancellationToken)
    {
        ...
        var count = await RealtimeApprovalsCounter.CountPendingForApproverAsync(
            _approvalChainsAdmin, _proxyDelegationsAdmin, domainEvent.TenantId, domainEvent.ApproverId, asOf, cancellationToken);

        var users = await _usersAdmin.ListIgnoringFiltersAsync(
            new UserByEmployeeIdSpecification(domainEvent.TenantId, domainEvent.ApproverId), cancellationToken);
