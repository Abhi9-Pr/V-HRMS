...
public sealed class ApproveRegularizationCommandHandler : IRequestHandler<ApproveRegularizationCommand, Result>
{
    ...
    public ApproveRegularizationCommandHandler(
    ...
    public async Task<Result> Handle(ApproveRegularizationCommand request, CancellationToken cancellationToken)
    {
        ...
        if (callerUser?.EmployeeId is null || authorizedApproverId is null || callerUser.EmployeeId != authorizedApproverId)
        {
            return Result.Failure(Error.Forbidden(
                "regularization_request.not_authorized_approver", "You are not authorized to approve this request."));
        }

        var result = regularizationRequest.Approve(callerUser.EmployeeId.Value, now);
        if (result.IsFailure)
        {
            return result;
        }
