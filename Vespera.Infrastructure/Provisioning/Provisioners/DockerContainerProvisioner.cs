...
public sealed class DockerContainerProvisioner : IDatabaseProvisioner
{
    ...
    public async Task<string> ProvisionAsync(CancellationToken cancellationToken)
    {
        ...
        var connectionString = BuildConnectionString(options.Engine, docker, hostPort);
        await WaitForReadinessAsync(options.Engine, connectionString, docker.StartupTimeoutSeconds, cancellationToken);
    ...
    private static string BuildConnectionString(DatabaseEngine engine, DockerDatabaseOptions docker, int hostPort) => engine switch
    {
        DatabaseEngine.Postgres =>
            $"Host=127.0.0.1;Port={hostPort};Username={docker.Credentials.Username};Password={docker.Credentials.Password};Database=postgres",
        DatabaseEngine.SqlServer =>
            $"Server=127.0.0.1,{hostPort};User Id=sa;Password={docker.Credentials.Password};Database=master;TrustServerCertificate=true",
        _ => throw new NotSupportedException($"Unsupported database engine '{engine}'."),
    };
}
