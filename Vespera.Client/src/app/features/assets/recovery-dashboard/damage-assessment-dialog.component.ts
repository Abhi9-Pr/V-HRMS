...
export interface DamageAssessmentDialogData {
  recoveryId: string;
}
...
export class DamageAssessmentDialogComponent {
  ...
  private readonly dialogRef = inject(MatDialogRef<DamageAssessmentDialogComponent>);

  readonly data = inject<DamageAssessmentDialogData>(MAT_DIALOG_DATA);
  ...
  submit(): void {
    ...
    this.assetsFacade
      .recordDamageAssessment(this.data.recoveryId, this.form.getRawValue())
      .pipe(
        map(() => true),
