...
export class DepartmentFormDialogComponent {
  ...
  readonly availableParents = this.data.availableParents.filter(
    (department) => department.id !== this.data.department?.id,
  );
    ...
    const request$ =
      this.data.mode === 'create'
        ? this.departmentsFacade
            .create({ name, code, parentDepartmentId: parentDepartmentId ?? undefined })
            .pipe(map(() => true))
        : this.departmentsFacade
