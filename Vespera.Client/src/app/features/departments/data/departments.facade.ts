...
export class DepartmentsFacade {
  ...
  load(query: DataTableQuery): void {
    ...
    // Named list2 by the generated client: "List" collides with several other controllers'
    // own action names across the API surface, and NSwag numbers the losing side of the tie —
    // see SwaggerServiceCollectionExtensions' CustomOperationIds doc comment.
    this.client.list2(query.page, query.pageSize, query.sortBy, query.sortDescending).subscribe({
      next: (result) => {
