...
describe('DepartmentsFacade', () => {
  let client: jest.Mocked<Pick<DepartmentsClient, 'list2' | 'create' | 'update' | 'delete'>>;
  let facade: DepartmentsFacade;

  beforeEach(() => {
    client = { list2: jest.fn(), create: jest.fn(), update: jest.fn(), delete: jest.fn() };
  ...
  it('load() should populate departments/totalCount on success', () => {
    ...
    expect(client.list2).toHaveBeenCalledWith(1, 20, undefined, false);
  });
  ...
  it('load() should populate error on failure and stop loading', () => {
    ...
    client.list2.mockReturnValue(throwError(() => apiError));
