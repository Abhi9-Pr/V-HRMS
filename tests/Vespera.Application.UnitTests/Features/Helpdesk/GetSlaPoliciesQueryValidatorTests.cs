...
public class GetSlaPoliciesQueryValidatorTests
{
    private readonly GetSlaPoliciesQueryValidator _validator = new();

    [Fact]
    public void Should_Have_Error_When_Page_Is_Not_Greater_Than_Zero()
    {
        var query = new GetSlaPoliciesQuery(new PagedRequest(0, 20));
    ...
    {
        var query = new GetSlaPoliciesQuery(new PagedRequest(1, 101));
    ...
    {
        var query = new GetSlaPoliciesQuery(new PagedRequest(1, 20));
