...
public class CloseTicketCommandHandlerTests
{
    ...
    private CloseTicketCommandHandler CreateHandler() => new(_tickets);
    ...
    [Fact]
    public async Task Handle_Should_Close_A_Resolved_Ticket()
    {
        var ticket = CreateTicket();
        ticket.Resolve(Now);

        var handler = CreateHandler();
        var result = await handler.Handle(new CloseTicketCommand(ticket.Id.Value, null), CancellationToken.None);
    ...
    [Fact]
    public async Task Handle_Should_Fail_When_The_Ticket_Is_Not_Yet_Resolved()
    {
        var ticket = CreateTicket();
    ...
    public async Task Handle_Should_Fail_When_The_Ticket_Does_Not_Exist()
    {
        ...
        var handler = CreateHandler();
        var result = await handler.Handle(new CloseTicketCommand(Guid.NewGuid(), null), CancellationToken.None);
