...
public class TicketSlaBreachedNotificationHandlerTests
{
    ...
    private TicketSlaBreachedNotificationHandler CreateHandler() => new(_tickets, _categories, _departments, _notificationDispatcher);
    ...
    private (Ticket Ticket, TicketCategory Category, Department Department) SeedTicketChain(EmployeeId? headEmployeeId)
    {
        ...
        var ticket = Ticket.Raise(
            ...
            TicketPriority.High, Now, TimeSpan.FromHours(1)).Value;
    ...
    public async Task Handle_Should_Notify_The_Department_Head_When_Everything_Resolves()
    {
        ...
        await handler.Handle(
            new DomainEventNotification<TicketSlaBreached>(new TicketSlaBreached(ticket.Id, _tenantId, ticket.DueAt, Now)), CancellationToken.None);
    ...
    public async Task Handle_Should_Do_Nothing_When_The_Ticket_Is_Not_Found()
    {
        ...
        var handler = CreateHandler();
        await handler.Handle(
            new DomainEventNotification<TicketSlaBreached>(new TicketSlaBreached(TicketId.New(), _tenantId, Now, Now)), CancellationToken.None);

        await _notificationDispatcher.DidNotReceiveWithAnyArgs().DispatchAsync(default!, default);
    }

    [Fact]
    public async Task Handle_Should_Do_Nothing_When_The_Category_Is_Not_Found()
    {
        var ticket = Ticket.Raise(
        ...
        _tickets.ListIgnoringFiltersAsync(Arg.Any<TicketByIdSpecification>(), Arg.Any<CancellationToken>()).Returns([ticket]);
        _categories.ListIgnoringFiltersAsync(Arg.Any<TicketCategoryByIdSpecification>(), Arg.Any<CancellationToken>()).Returns(new List<TicketCategory>());
