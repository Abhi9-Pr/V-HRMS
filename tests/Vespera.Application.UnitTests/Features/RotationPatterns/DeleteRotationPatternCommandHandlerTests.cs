...
public class DeleteRotationPatternCommandHandlerTests
{
    ...
    public DeleteRotationPatternCommandHandlerTests()
    {
    ...
    private DeleteRotationPatternCommandHandler CreateHandler() =>
        new(_rotationPatterns, _tenantContext, _currentUser, _dateTimeProvider);

    [Fact]
    public async Task Handle_Should_Soft_Delete_When_Pattern_Exists()
    {
        var pattern = RotationPattern.Create(
            _tenantId, "Pattern", [new RotationPatternDay(0, null)], DateTimeOffset.UtcNow, "seed").Value;
        ...
        var result = await handler.Handle(new DeleteRotationPatternCommand(pattern.Id.Value), CancellationToken.None);

        result.IsSuccess.Should().BeTrue();
        pattern.IsDeleted.Should().BeTrue();
    }
    ...
    public async Task Handle_Should_Return_NotFound_When_Pattern_Missing()
    {
        ...
        var handler = CreateHandler();
        var result = await handler.Handle(new DeleteRotationPatternCommand(Guid.NewGuid()), CancellationToken.None);
