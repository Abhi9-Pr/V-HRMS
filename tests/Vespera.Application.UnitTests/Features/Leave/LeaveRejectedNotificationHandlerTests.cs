...
public class LeaveRejectedNotificationHandlerTests
{
    ...
    private LeaveRejectedNotificationHandler CreateHandler() => new(_usersAdmin, _dispatcher);

    private static LeaveRejected CreateEvent(EmployeeId employeeId) =>
        new(LeaveRequestId.New(), TenantId, employeeId, EmployeeId.New(), "Not eligible", Now);
    ...
    public async Task Handle_Should_Dispatch_A_Notification_To_The_Requesting_Employees_User()
    {
        ...
        var notification = new DomainEventNotification<LeaveRejected>(CreateEvent(employeeId));
        await CreateHandler().Handle(notification, CancellationToken.None);

        await _dispatcher.Received(1).DispatchAsync(
            Arg.Is<NotificationMessage>(m => m.RecipientId == user.Id.Value.ToString() && m.Body.Contains("Not eligible")),
    ...
    public async Task Handle_Should_Do_Nothing_When_The_Employee_Has_No_Linked_User()
    {
        ...
        var notification = new DomainEventNotification<LeaveRejected>(CreateEvent(EmployeeId.New()));
        await CreateHandler().Handle(notification, CancellationToken.None);
