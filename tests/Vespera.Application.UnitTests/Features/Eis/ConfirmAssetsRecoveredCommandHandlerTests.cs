...
public class ConfirmAssetsRecoveredCommandHandlerTests
{
    ...
    public ConfirmAssetsRecoveredCommandHandlerTests()
    {
    ...
    private ConfirmAssetsRecoveredCommandHandler CreateHandler() => new(_checklists, _tenantContext, _currentUser, _dateTimeProvider);
    ...
    [Fact]
    public async Task Handle_Should_Mark_Assets_Recovered_When_Checklist_Exists()
    {
        ...
        var result = await CreateHandler().Handle(new ConfirmAssetsRecoveredCommand(checklist.EmployeeId.Value), CancellationToken.None);
    ...
    public async Task Handle_Should_Return_NotFound_When_No_Checklist_Exists()
    {
        ...
        var result = await CreateHandler().Handle(new ConfirmAssetsRecoveredCommand(Guid.NewGuid()), CancellationToken.None);
    ...
    {
        var checklist = CreateChecklist();
        checklist.MarkAssetsRecovered(DateTimeOffset.UtcNow, "hr@vespera.test");
        _checklists.FirstOrDefaultAsync(Arg.Any<ISpecification<OffboardingChecklist>>(), Arg.Any<CancellationToken>()).Returns(checklist);
