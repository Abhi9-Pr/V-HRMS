...
public class GetDesignationsQueryValidatorTests
{
    private readonly GetDesignationsQueryValidator _validator = new();

    [Fact]
    public void Should_Have_Error_When_Page_Is_Less_Than_One()
    {
        var query = new GetDesignationsQuery(new PagedRequest { Page = 0, PageSize = 20 });
    ...
    {
        var query = new GetDesignationsQuery(new PagedRequest { Page = 1, PageSize = 500 });
    ...
    {
        var query = new GetDesignationsQuery(new PagedRequest { Page = 1, PageSize = 20 });
