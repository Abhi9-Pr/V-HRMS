...
public class WithdrawCandidateCommandHandlerTests
{
    ...
    private WithdrawCandidateCommandHandler CreateHandler() => new(_candidateReads, _candidates);
    ...
    [Fact]
    public async Task Handle_Should_Withdraw_An_Active_Candidate()
    {
        var candidate = CreateCandidate();

        var result = await CreateHandler().Handle(new WithdrawCandidateCommand(candidate.Id.Value, null), CancellationToken.None);
    ...
    public async Task Handle_Should_Fail_When_Candidate_Not_Found()
    {
        ...
        var result = await CreateHandler().Handle(new WithdrawCandidateCommand(Guid.NewGuid(), null), CancellationToken.None);
